...
function show_db(){
	$connect = mysqli_connect("p:localhost", "root", "mysql", "cloudice_kniexam");
	if(!$connect){
...
function logvalidate($uid, $page){
	if(!isset($_SESSION['session_id'])){
		...
		$sql = 'select * from user where userid="'.$_SESSION['username'].'"';
		//echo $sql;
		$result = execute_query($sql,dbconnect());
		if(mysqli_num_rows($result)!=0){
			$sql = 'select * from session_admin where s_id="'.$_SESSION['session_id'].'" and user="'.$_SESSION['username'].'" and s_start_date="'.date("Y-m-d").'" order by sno desc limit 1';
			//echo $sql;
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Welcome To Janta Inter College!</title>
<link href="css/style.css"  rel="stylesheet" type="text/css" media="all" />
<link href="scripts/index.css" rel="stylesheet" type="text/css" media="all" />
<link href="css/form.css" rel="stylesheet">
<link rel="stylesheet" href="themes/base/jquery.ui.all.css">
