...
$msg='';
// print_r($_POST);
if(isset($_POST['eid']) and $_POST['eid']!=""){
	$sqls="SELECT * FROM exam_copy_view WHERE sno='".$_POST['eid']."'";
	$ress=mysqli_query($db,$sqls);
	$row=mysqli_fetch_assoc($ress);
// 	print_r($row);
	$_POST['exam_roll_no']=$row['exam_roll_no'];
	$_POST['result_course']=$row['course'];
	$_POST['papercode']=$row['papercode'];
	$_POST['mobile_no']=$row['mobile_no'];
}
...
<html lang="en">
  ...
  <body class="w-100 m-auto" id="main">
	...
	<form action="confirm_answer_sheet_view.php" method="POST">
			...
			<div class="container-fluid">
				...
				<table class="table text-center" style="border:1px solid black; ">
						...
						$paperCodeArray = array();
							...
							while ($row2 = mysqli_fetch_assoc($result2)) {
								if (!in_array($row2['paper_code'], $paperCodeArray)) {
									...
									$checked="";
									if(isset($_POST['eid']) and $_POST['eid']!=""){
										if($_POST['papercode']==$row2['paper_code']){
											$checked="checked";
										}
									}
					?>
					<tr>
							...
							<td><input type="radio" name="papercode" class="checkbox form-check-input" value="<?php echo $row2['paper_code']; ?>" onclick="getCheckboxValues()" <?php echo $checked;?>></td>
					</tr>
						<?php
				...
				<div class="lead"><input type="checkbox" class="form-check-input" name="abc" required <?php if(isset($_POST['eid']) and $_POST['eid']!=""){echo "checked";} ?>> &nbsp;&nbsp;&nbsp; 
		...
		<?php
			if(isset($_POST['eid']) and $_POST['eid']!=""){
				?>
				<input type="hidden" class="" value="<?php echo $_POST['eid'];?>" name="editid">

				<input type="submit" class="btn btn-primary m-5" value="Submit" name="edit">
				
				<?php
			}else{
				?>
				<input type="submit" class="btn btn-primary m-5" value="Submit" name="submit">
				<?php
			}
	...
	function getCheckboxValues() {
		...
		for (var i = 0; i < checkboxes.length; i++) {
			if (checkboxes[i].checked) {
				totalPayableAmount += 2500; // Assuming each checkbox adds 3000 to the total
			}
		}
