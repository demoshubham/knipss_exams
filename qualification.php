		...
		if(isset($_POST['qualification_no']) ){
			for($i=1; $i<=$_POST['qualification_no']; $i++){
				if($_POST['part_desc'.$i.'_board']!='' ){
					$sql = execute_query('insert into admission_qualification(d_student_info_sno,name_of_examination,board_university_name,college_name,year,roll_no,obtained_marks,total_marks,percentage,division,cgpa,status,created_by,creation_time)values("'.
					$id.'","'.
					$_POST['part_desc'.$i].'","'.
	...
	<div class="card">
					...
					<div class=" card card-body col-md-11 my-auto mx-auto" style="border-top-color: #d2d6de; background-color:whitesmoke;" >
						...
						<div class="row mt-1">
							<div class="col-md-6">	
								...
								<input type="text" name="caste" id="caste" class="form-control " value="" pattern=[0-9]{10} minlength="10" maxlength="10"  required />
							</div>
							<div class="col-md-6">							
								<label>PARENT'S INCOME (Annual)</label>
								<input type="text" name="parent_income" id="parent_income" class="form-control " value="" 	/>
							</div>
						</div>
						<div class="row mt-1">
							...
							<div class="col-md-6">							
								...
								<select name="mother_tongue" id="mother_tongue" class="form-control" required>
									<option value="hindi" <?php echo (isset($_GET['id']) && isset($data['mother_tongue']) && $data['mother_tongue']=='hindi') ? 'selected="selected"' : ''; ?>>Hindi</option>
									<option value="english" <?php echo (isset($_GET['id']) && isset($data['mother_tongue']) && $data['mother_tongue']=='english') ? 'selected="selected"' : ''; ?>>English</option>
		...
		<table width="100%" class="table table-striped-success table-hover rounded ">
			<tr class="bg-secondary text-white" style='background-color: #F5F5F5 ;'>
				...
				<th width="5%">Year of Passing</th>
				<th width="10%">Roll No</th>
				
				<th width="10%">Select</th>
				<th width="5%">Obtained Marks</th>
				<th width="5%">Total Marks</th>
				<th width="5%">Percentage/CGPA</th> 
        ...
        ?>
				...
				<td width="10%"><select name="" id="select<?php echo $i; ?>" class="form-control" tabindex="<?php echo $tab++; ?>" onchange="toggleFields(<?php echo $i; ?>)">
...
<td><input name="part_desc<?php echo $i; ?>_cgpa" type="text" value="<?php if(isset($_POST['part_desc'.$i.'_cgpa'])){echo $_POST['part_desc'.$i.'_cgpa'];} ?>" class="fieldtextmedium" maxlength="6" id="<?php echo $i ?>_cgpa" /></td> 
		...
		<table>
			...
			<center><button type="submit" class="btn btn-danger "  name="submit" value="Submit" onClick="return confirmSubmit()">Submit</button><center>
		</form>
