            ...
            <div class="col-md-12">
                <div class="card">
					...
					<table class="table table-striped table-hover table-bordered" id="general_stat_table">
						...
						if($_POST['order_id']!=''){
						    $i=1;
						    $sql = 'select * from online_payment where order_id="'.$_POST['order_id'].'"';
							...
							while($row_trans = mysqli_fetch_assoc($result_trans)){
							    $sql1 = 'SELECT * FROM register_users WHERE sno='.$row_trans['student_id'];
								...
								if($details[$row_trans['order_id']][36]=='Shipped'){
									...
									if($row_trans['order_status']!='Success'){
									    $sql = 'update online_payment set order_status="Success" where sno="'.$row_trans['sno'].'"';
									    execute_query($sql);
									    $sql = 'update online_payment set 
									...
									/*$chk .= $sql = 'update online_payment set 
                                        tracking_id = "'.$details[$row_trans['order_id']][0].'", 
                                        bank_ref_no = "'.$details[$row_trans['order_id']][48].'", 
                                        order_status = "Success", 
                                        failure_message = "", 
    							...
    							while($row_trans = mysqli_fetch_assoc($result_trans)){
    								...
    								if($details[$row_trans['order_id']][36]=='Shipped'){
    									...
    									if($row_trans['order_status']!='Success'){
                                            ...
                                            transaction_no="'.$details[$row_trans['order_id']][0].'",
                                            payment_status="Success"
                                            where sno='.$row_trans['student_id'];
                                            //echo $sql;
                                            execute_query($sql);
                                            if(mysqli_error($db)){
                                                die("Error : ".mysqli_error($db)." >> ".$sql);
